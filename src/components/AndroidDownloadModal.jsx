...
export default function AndroidDownloadModal({ isOpen, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md animate-in fade-in">
      <div className="relative w-full max-w-lg glass-panel bg-slate-950 rounded-3xl border border-[#00FF75]/40 p-8 space-y-6 shadow-2xl overflow-hidden">
        ...
        <div className="text-center space-y-3">
          <div className="w-16 h-16 rounded-2xl bg-emerald-500/20 border border-[#00FF75]/40 flex items-center justify-center text-[#00FF75] mx-auto shadow-lg shadow-emerald-500/20">
        ...
        <div className="space-y-3">
          ...
          <p className="text-[11px] text-center text-slate-400 flex items-center justify-center gap-1">
            <ShieldCheck className="w-3.5 h-3.5 text-[#00FF75]" /> Verified 100% Free • No Subscription Required
          </p>
        </div>

        <div className="relative flex items-center justify-center">
          <div className="border-t border-white/10 w-full"></div>
          <span className="bg-slate-950 px-3 text-[11px] font-bold text-slate-500 uppercase tracking-wider">or get launch alert</span>
          <div className="border-t border-white/10 w-full"></div>
        </div>
        ...
        {submitted ? (
          <div className="p-5 rounded-2xl bg-emerald-950/60 border border-emerald-500/40 text-center space-y-2">
            <CheckCircle className="w-8 h-8 text-[#00FF75] mx-auto animate-bounce" />
            <h4 className="text-sm font-bold text-white">You're on the VIP Launch List!</h4>
            <p className="text-xs text-slate-300">
              We'll send an instant download alert to <strong className="text-emerald-400">{email}</strong> as soon as Google Play approval is complete!
            </p>
            <button
              onClick={onClose}
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-slate-300">Enter Email for Direct Launch Link &amp; Free AI Credits</label>
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 bg-slate-900 border border-white/20 focus:border-[#00FF75] rounded-xl px-4 py-3 text-sm text-white placeholder-slate-500 focus:outline-none"
                />
                <button
                  type="submit"
                  disabled={loading}
                  className="px-5 py-3 rounded-xl bg-white/10 hover:bg-white/20 border border-white/20 text-white font-bold text-sm flex items-center justify-center gap-1.5 transition-all disabled:opacity-50"
                >
